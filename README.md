...
```
However, since a state should have minimal knowledge about the inner workings of it's holder, it is best to limit calls to holder to #transition_to.

#### version 2.0.0
The setter __hx_size_limit=__ is no longer supported.

A state holder should limit mutability to @current_state and @state_history. Managing current state and maintaining state history are it's responsibilities. The history size limit, @hx_size_limit, is not the state holder's responsibility and should not change after creation of the state holder. If any thing more complicated than maintaining a list of recent state transitions were required, state history could be split out to separate class. Similarly, if dynamically changing the size of the state history list is desirable, that responsibility should be split out to a separate class. These refactorings may occur in a future version.
...
StateHolder and State are not expected to be used directly. Rather, they are intended to be inherited from. The child state holder should provide methods not specific to its current state and leave methods specific to the current state to that state. The public methods of a child state act as receivers of event messages via delegation from the state holder. Such events may cause effects that are managed by the current state and may also cause transition to a new state. State change logic is expected to be held within the current state. Two private methods, __#enter__ and __#exit__, *must* be provided by each state. These are called by the state holder __#transition_to__ method at the appropriate points in the state life cycle. Neither __#enter__ nor __#exit__ nor any other private method of a state are intended to be called by a user of the state holder. To avoid potentially stepping on the user's code __#enter__ and __#exit__ are called with \_\_send\_\_.
